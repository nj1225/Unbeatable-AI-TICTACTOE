...
class GeniusComputerPlayer(Player):
    ...
    def get_move(self, game):
        if len(game.available_moves()) == 9:
            square = random.choice(game.available_moves())  # randomly chossing a move
        else:
            # get the square based off the minmax algorithnm
            square = self.minimax(game, self.letter)['position']
        return square

    def minimax(self, state, player):
        # here we used the state instead of games bcz we are sending a screenshot of the state in each iteration
        max_player = self.letter  # yourself
        other_player = 'o' if player == 'x' else 'x'  # the other player ...so whatever letter is

        # first, we want to check if the previous move is a winnner
        # this is our base case
        if state.current_winner == other_player:
            # we should return position and score beacuase we need to keep track of the score
            # for minimax to work
        ...
        for possible_move in state.available_moves():
            ...
            sim_score['position'] = possible_move  # otherwise this will get messed up from the recursive

            # step 4: update the dictonaries if neccessary
